...
contract OffchainResolver is IExtendedResolver, ERC165, Ownable {
    ...
    event URLUpdated(address[] signers);
    ...
    constructor(string memory _url, address[] memory _signers) {
        url = _url;
        doUpdateSigners(_signers, true);
    }

    function updateURL(string calldata _url) public onlyOwner {
        url = _url;
    }

    function updateSigners(address[] memory _signers, bool add) public onlyOwner {
        doUpdateSigners(_signers, add);
    }
    ...
    function doUpdateSigners(address[] memory _signers, bool add) private {
        for (uint256 i = 0; i < _signers.length; i++) {
            signers[_signers[i]] = add;
        }
